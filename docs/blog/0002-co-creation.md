...
- **Ideas seeded** — mission, values, default SOPs, regulatory tracker (when relevant). The reference shape: aeqi ships with dozens of seed Ideas out of the gate.
- **Events scheduled** — daily standups, weekly reviews, monthly compliance checks, quarterly board prep. A few dozen in the aeqi reference. Paused by default; you flip them on as you need them.
